# Setting to do a pipe run whose main purpose is to get Colony
# setup to run.  However, we also might want to show how our levels
# of HWD and LD are reduced by the sibyanking.  So, let's do that.
...
PREPARE_STRUCTURE_AREA=0;   # make a directory for Structure stuff.

PREPARE_GENETIX_DIR=0;      # make a directory that can be run in Genetix
